...
For **adding `capture()` / client instrumentation** in application code, follow that repo’s existing PostHog patterns (e.g. a shared analytics module or `posthog-js` / framework SDK). Use a **posthog-instrumentation**-style skill when the user only wants code-level tracking without MCP.
...
## Aligning MCP with your codebase

Use MCP for **PostHog project configuration and querying**; use **application code** for what your app sends. Keep names consistent:

- Client event helpers and `capture` call sites (often a small `posthog` or `analytics` module).
- Server-side capture, if any (Node SDK or HTTP API).
- Feature-flag reads in the app vs flag definitions in PostHog.
- CSP `connect-src` / `script-src` allowlists and env vars (`NEXT_PUBLIC_POSTHOG_*`, etc.) if you use the browser SDK.

When creating actions, cohorts, or insights via MCP, cross-check event and property strings against the codebase so definitions match production telemetry.
...
- [ ] Cross-check important event names against the app’s PostHog instrumentation.

## Further detail

- [reference.md](reference.md) — links, example prompts, installation notes.
